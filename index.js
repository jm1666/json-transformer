...
function parseFile(entry, DICT_US, DICT_UK, callback) {
    if (JSON.parse(JSON.stringify(entry)).errno) {
        ...
        if (program.spell) {
            ...
            var L1checked = _.map(data, function (d, k) {
            ...
            var L2filtering = _.reject(L1checked, function (d) {
                return d == false
            });

            callback && callback(JSON.stringify(L2filtering, null, 2));
        }

        if (!(program.spell)) {
            callback && callback(JSON.stringify(data, null, 2));
        }
    }
}
